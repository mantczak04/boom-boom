# -*- coding: utf-8 -*-
# Adapted from Kaixhin/Rainbow memory.py (MIT).
# Float32 (C, H, W) transitions; no /255 scaling; history_length = 1.
...
def make_transition_dtype(state_shape: tuple[int, int, int]) -> np.dtype:
    return np.dtype(
        [
            ("timestep", np.int32),
...
def make_blank_transition(state_shape: tuple[int, int, int]) -> tuple:
    return (
        0,
...
class ReplayMemory:
    def __init__(self, args, capacity):
        ...
        state_shape = (args.obs_channels, args.board_height, args.board_width)
        self._state_shape = state_shape
        transition_dtype = make_transition_dtype(state_shape)
        self._blank_trans = make_blank_transition(state_shape)
        self.transitions = SegmentTree(capacity, transition_dtype, self._blank_trans)

    def _state_to_numpy(self, state: torch.Tensor) -> np.ndarray:
        if state.dim() == 4:
            state = state[-1]
        return state.to(dtype=torch.float32, device=torch.device("cpu")).numpy()

    # Adds state and action at time t, reward and terminal at time t + 1
    def append(self, state, action, reward, terminal):
        stored = self._state_to_numpy(state)
        self.transitions.append(
            (self.t, stored, action, reward, not terminal),
    ...
    def _get_samples_from_segments(self, batch_size, p_total):
        ...
        states = torch.tensor(all_states[:, self.history - 1], device=self.device, dtype=torch.float32)
        next_states = torch.tensor(all_states[:, self.n + self.history - 1], device=self.device, dtype=torch.float32)
    ...
    def __next__(self):
        ...
        transitions[blank_mask] = self._blank_trans
        state = torch.tensor(transitions["state"][-1], dtype=torch.float32, device=self.device)
        self.current_idx += 1
        return state
